...
- (void)initAxisY {
    ...
    for (NSUInteger i = 0; i < self.latitudeNum; i++) {
        if (self.axisCalc == 1) {
            ...
            NSString *value = [NSString stringWithFormat:@"%.7f",degree];
    ...
    if (self.axisCalc == 1) {
        ...
        NSString *value = [NSString stringWithFormat:@"%-.7f", (self.maxValue) / self.axisCalc];
        [TitleY addObject:value];
    }
    ...
    if (self.axisYPosition==CCSGridChartYAxisPositionLR) {
        ...
        for (NSUInteger i = 0; i < self.latitudeNum; i++) {
            if (self.axisCalc == 1) {
                float degree = (self.minValue + i * average) / self.axisCalc;
                NSString *value = [NSString stringWithFormat:@"%.2f%%",((degree-self.closeValue)/self.closeValue)*100];
                [TitleY addObject:value];
        ...
        if (self.axisCalc == 1) {
            ...
            NSString *value = [NSString stringWithFormat:@"%.2f%%",((((self.maxValue) / self.axisCalc)-self.closeValue)/self.closeValue)*100];
...
//                    self.displayCrossYOnTouch = YES;
                    ...
                    if (self.tag==1234567890) {
                        result = ((CCSLineData *) [line.data objectAtIndex:self.selectedIndex]).date;
                    }
                    else{
