...
//    float average = ((self.maxValue - self.minValue) / self.latitudeNum);
//    //处理刻度
//    for (NSUInteger i = 0; i < self.latitudeNum; i++) {
//        if (self.axisCalc == 1) {
//            float degree = (self.minValue + i * average) / self.axisCalc;
//            
//            
//           // NSString *value = [NSString stringWithFormat:@"%.2f",degree];
//            
//            NSString *value = nil;
//            if (degree > 10) {
//                value  = [NSString stringWithFormat:@"%.0f",degree];
//            } else {
//                value = [NSString stringWithFormat:@"%.6f",degree];
//            }
//            
//            
//            
...
//    //处理最大值
//    if (self.axisCalc == 1) {
//        float degree = (self.maxValue) / self.axisCalc;
//        // NSString *value = [NSString stringWithFormat:@"%.2f",degree];
//        
//        NSString *value = nil;
//        if (degree > 10) {
...
- (void)initAxisY {
    ...
    GXLog(@"最大值减去最小值 %f", self.maxValue - self.minValue);
    GXLog(@"浮动的差值 %f",average);
    ...
    for (NSUInteger i = 0; i < self.latitudeNum; i++) {
        ...
        if (self.axisCalc == 1) {
            ...
            float degree = (self.minValue + i * average) / self.axisCalc;
            NSString *value;
            value = [NSString stringWithFormat:@"%g",degree];
            
            if (average > 10) {
                value = [NSString stringWithFormat:@"%.0f",degree];
            } else if (average < 10 && average > 2) {
                value = [NSString stringWithFormat:@"%.2f",degree];
            } else if (average < 2 && average > 0.1) {
                value = [NSString stringWithFormat:@"%.2f",degree];
            } else if (average < 0.1 && average > 0.01) {
                value = [NSString stringWithFormat:@"%.2f",degree];
            }  else {
                value = [NSString stringWithFormat:@"%.4f",degree];
            }
...
//                 //NSLog(@"%f",fabs(pt1.x - self.singleTouchPoint.x));
    ...
    if ([allTouches count] == 1) {
        if (_flag == 0) {
           ...
           // float stickWidth = ((self.frame.size.width - self.axisMarginLeft - self.axisMarginRight) / self.displayNumber) - 1;
            ...
            if (self.displayCrossXOnTouch==NO) {
                          if (pt1.x >_firstX) {
                NSLog(@"右移动");
                ...
                {
                    if (self.displayFrom + self.displayNumber + 1 <= [self.stickData count]) {
                        self.displayFrom = self.displayFrom + 1;
                    }
...
- (void) setDisplayFrom:(NSUInteger)displayFrom
{
    ...
    if (self.chartDelegate && [self.chartDelegate respondsToSelector:@selector(CCSChartDisplayChangedFrom:number:type:)]) {
        [self.chartDelegate CCSChartDisplayChangedFrom:displayFrom number:self.displayNumber type:self.type];
    }
}
...
-(void) setDisplayNumber:(NSUInteger)displayNumber
{
    ...
    if (self.chartDelegate && [self.chartDelegate respondsToSelector:@selector(CCSChartDisplayChangedFrom: number:type:)]) {
        [self.chartDelegate CCSChartDisplayChangedFrom:self.displayFrom number:displayNumber type:self.type];
    }
}
