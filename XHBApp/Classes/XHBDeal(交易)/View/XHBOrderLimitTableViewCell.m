...
@implementation XHBOrderLimitTableViewCell
{
    //列表名字与编码
    UILabel *lb_name;
    //方向
...
+ (instancetype)cellWithTableView:(UITableView *)tableView {
    ...
    XHBOrderLimitTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:Identifier];
    
    if (!cell) {
        cell = [[XHBOrderLimitTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:Identifier];
        
        //cell箭头图片
        UIImageView *cellindimg=[[UIImageView alloc]initWithFrame:CGRectMake(GXScreenWidth-31, (100-16)/2.0, 16, 16)];
...
- (void)setPModel:(XHBTradePositionModel *)pModel {
    ...
    lb_OrderLongShort.frame=CGRectMake(15, 15 + (25-18)/2.0, 64, 18);
    ...
    [lb_name sizeToFit];
    lb_name.frame=CGRectMake(lb_OrderLongShort.frame.origin.x+lb_OrderLongShort.frame.size.width+10, 15, lb_name.frame.size.width, 25);
    ...
    if(!lb_tit_OrderClosePrice)
    {
        ...
        lb_tit_OrderClosePrice.text=@"现在价格";
