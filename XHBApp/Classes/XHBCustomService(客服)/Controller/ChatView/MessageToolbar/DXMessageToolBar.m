...
//    textViewLeftMargin += CGRectGetMaxX(self.questionButton.frame);
    ...
    //表情
    self.faceButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.frame) - kInputTextViewMinHeight - kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
    ...
    // 输入框的高度和宽度
  //  CGFloat width = CGRectGetWidth(self.bounds) - (allButtonWidth ? allButtonWidth : (textViewLeftMargin));
    CGFloat width = CGRectGetWidth(self.frame) - kHorizontalPadding - textViewLeftMargin - CGRectGetWidth(self.faceButton.frame) - kHorizontalPadding;
    
    // 初始化输入框
    self.inputTextView = [[XHMessageTextView  alloc] initWithFrame:CGRectMake(kHorizontalPadding, kVerticalPadding, width, kInputTextViewMinHeight)];
