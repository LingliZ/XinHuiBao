...
//    }
    if([GXUserInfoTool isLogin])
    {
        ...
        {
            //登录服务器返回的账号
            NSDictionary*accountDic=[[NSDictionary alloc]initWithDictionary:[GXUserInfoTool getEaseMobAccoutAndPassword]];
            NSString*account=[[NSString alloc]initWithString:accountDic[EaseMobAccount]];
            NSString*password=[[NSString alloc]initWithString:accountDic[EaseMobPassword]];
//            [self LoginEaseMobWith:account password:password];
            [GXPushTool LoginEaseMobWith:account password:password];
    //[self LoginEaseMobWith:account password:password];
        }
    }
    else
...
-(void)addItemForAddCount
{
    ...
    [rightButtonItem setTitleTextAttributes:@{NSFontAttributeName:GXFONT_PingFangSC_Light(GXFONT_SIZE14),NSForegroundColorAttributeName:UIColorFromRGB(0xFFFFFF)} forState:UIControlStateNormal];
...
//    if ([GXUserInfoTool isLogin]) {
//        GXAddCountViewController*addCountVC=[[GXAddCountViewController alloc]init];
...
//        loginVC.recieveSiteUrl = GXOnlineCSSite;
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (cell == nil) {
                ...
                if (model.isSender) {
                    ...
                    cell.headImageView.image = [UIImage imageNamed:[GXUserInfoTool isLogin] ? [GXUserInfoTool getUserHeadImageName] : UserDefineHeadImage];
                }
                else{
...
{
#pragma mark--仅聊天相关的消息显示在改界面，不显示其他的推送消息
    if(message.ext)
    {
...
//    NSString *nickName = isLogin ? [GXUserInfoTool getUserNickName]:@"游客";
    NSString *nickName = isLogin ? [GXUserInfoTool getUserReallyName]:@"游客";
